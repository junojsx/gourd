...
struct ProductDetailView: View {
    ...
    private let initialItem: PantryItem
    ...
    /// Always reflects the latest state from the repo.
    private var item: PantryItem {
        repo.items.first { $0.id == itemId } ?? initialItem
    }
    ...
    private var storageTip: (icon: String, title: String, body: String) {
        ...
        case .leftovers:
            return ("takeoutbag.and.cup.and.straw.fill", "Refrigerate within 2 hours",
                    "Store in an airtight container in the fridge and consume within 3–4 days for best safety.")
        }
    }
    ...
    var body: some View {
        ...
        .onChange(of: item.quantity) { _, newQty in
            quantity = newQty
        }
        ...
        VStack(alignment: .leading, spacing: 12) {
            HStack(alignment: .top, spacing: 10) {
                ...
                Text(item.category.displayName.uppercased())
                    ...
                    .padding(.vertical, 6)
                    .background(Capsule().fill(Color.ftPrimaryBg))
                    .padding(.top, 2)
            }
        ...
        Button(action: {
            ...
            .background(
                RoundedRectangle(cornerRadius: 14)
                    .fill(quantity <= 0.5 ? Color.ftCrimson : Color.ftPrimaryBg)
            )
        }
