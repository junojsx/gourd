...
struct EditRecipeView: View {
        ...
        .onChange(of: selectedPhoto) { _, newItem in
            Task {
                if let data = try? await newItem?.loadTransferable(type: Data.self) {
                    ...
                    // Show preview immediately using a local path
    ...
    private var saveButton: some View {
        Button(action: {
            ...
            isSaving = true
            Task {
                var finalImageURL = imageSource
                if let data = selectedImageData {
                    do {
                        finalImageURL = try await recipeRepo.uploadRecipeImage(data, recipeId: recipe.id)
                    } catch {
            ...
            Group {
                if isSaving {
                    ProgressView().tint(Color.ftWarmBeige)
                } else {
                    Text("Save Changes")
            ...
            .background(
                RoundedRectangle(cornerRadius: 14)
                    .fill(isValid ? Color.ftPrimaryBg : Color.ftPrimaryBg.opacity(0.3))
            )
        }
