    ...
    private var generateButton: some View {
        VStack(spacing: 0) {
            ...
            Button(action: generateRecipe) {
                ...
                .padding(.horizontal, 20)
                .padding(.vertical, 16)
                .background(RecipeRateLimiter.canGenerate(isPro: subscriptions.isProSubscriber) ? Color.ftPrimaryBg : Color.ftPrimaryBg.opacity(0.35))
            }
        }
        .background(Color.ftWarmBeige)
    }
            ...
            if let onSave, let onDiscard {
                ...
                HStack(spacing: 12) {
                    Button(action: {
                        ...
                        .background(
                            RoundedRectangle(cornerRadius: 12)
                                .fill(isSaved ? Color.ftOlive : Color.ftPrimaryBg)
                        )
                    }
                    .disabled(isSaved)
